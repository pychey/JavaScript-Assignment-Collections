 ...
 */
const [firstUser, ...restUsers] = userData;
const { name, tasks: [firstTask] } = firstUser;
console.log('First User: ' + name);
console.log('First Task: ' + firstTask.description);
...
console.log('\nUsers other than Bob:', usersWithoutBob);
...
const usersWithIncompleteTasks = userData.filter(user =>
    user.tasks.find(task => !task.completed)
);

console.log('\nUsers with incomplete tasks:');
usersWithIncompleteTasks.forEach(user => {
    const incompleteTasks = user.tasks.filter(task => !task.completed);
    const incompleteCount = incompleteTasks.length;
    console.log('- ' + user.name + ' (' + incompleteCount + ' incomplete task)');
});
...
const updatedUserData = userData.map(user => {
    if (user.id === 2) {
        return {
            ...
            tasks: [
                ...user.tasks,
                { taskId: 203, description: 'Mentor junior staff', completed: false }
            ]
        };
    }
    return user;
...
console.log('\nUpdated User Data:', JSON.stringify(updatedUserData, null, 2));