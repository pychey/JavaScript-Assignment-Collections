...
const { description: firstTaskDescription } = firstUser.tasks[0];
console.log(firstName);
console.log(firstTaskDescription);
...
const usersWithIncompleteTasks = userData.filter(user => {
  return user.tasks.find(task => task.completed === false);
});
usersWithIncompleteTasks.forEach(user => {
  const incompleteCount = user.tasks.filter(task => task.completed === false).length;
