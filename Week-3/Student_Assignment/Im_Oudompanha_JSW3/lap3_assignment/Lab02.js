...
if (!product) console.log("Product don't exist.");
else console.log("Found product: ", product);
...
const findElectronic = products.filter(e => e.category == 'Electronics');
if (!findElectronic) console.log("Product don't exist.");
else console.log("Electronics products: ");
findElectronic.forEach(product => console.log('- ', product.name));
...
let total = 0;
cart.forEach(c => {
  const find = products.find(p => p.id == c.productId);
  if (find) total += find.price * c.quantity;
})
