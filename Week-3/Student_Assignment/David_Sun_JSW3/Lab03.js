...
console.log("First Task: ", user1.tasks[0].description);
...
const otherThanBob = userData.filter(noBob => noBob.id !== 2);
console.log(otherThanBob);
...
const incompleteUsers = userData.filter(user => user.tasks.find(task => !task.completed));
...
const updatedUserData = userData.map(user => 
    ...
    {...user, role: 'Senior Manager', 
        tasks: [...user.tasks, {taskId: 203, description: 'Mentor junior staff', completed: false}]
    }
    : user
)
