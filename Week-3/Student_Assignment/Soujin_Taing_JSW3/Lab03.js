...
const { name, tasks: [{ description }] } = firstUser;

console.log("First User:", name);
console.log("First Task:", description);
...
const usersWithoutBob = userData.filter(user => user.name !== "Bob");
console.log("Users other than Bob:", usersWithoutBob);
...
const usersWithIncompleteTasks = userData.filter(user =>
    user.tasks.find(task => task.completed === false) );
console.log("Users with incomplete tasks:");
usersWithIncompleteTasks.forEach(user => {
    const incompleteCount = user.tasks.filter(task => !task.completed).length;
    console.log(`- ${user.name} (${incompleteCount} incomplete task${incompleteCount > 1 ? "s" : ""})`);
});
...
const updatedUserData = userData.map(user => {
    if (user.id === 2) {
        return {
            ...user, // copy everything from Bob
            role: "Senior Manager", // update role
            tasks: [
                ...user.tasks, // keep existing tasks
                { taskId: 203, description: "Mentor junior staff", completed: false } // add new task
            ]
        };
    }
