...
const found=products.find((names)=> (names.name="laptop"));
console.log(found);
...
const product=products.filter(thing=>thing.category==='Electronics');
console.log(product);
...
let totalcart=0;
cart.forEach(item=>{
    const product=products.find(pd=>pd.id===item.productId);
    if(product){
        totalcart += product.price*item.quantity;}
    })
    console.log("Total Vart Price: $"+totalcart);
