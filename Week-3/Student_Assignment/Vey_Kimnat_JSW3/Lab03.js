...
const [{name: firstUser, tasks: [{description: firstDescription}]}] = userData;
console.log("First User: ", firstUser);
console.log("First Task: ", firstDescription);
/**
...
const removeBob = userData.filter(p => p.name != 'Bob');
if (removeBob) console.log("Users other than Bob: ",removeBob);
/**
...
findIncompletedTask.forEach(user => {
  const countIncompleteTask = user.tasks.filter(task => !task.completed).length;
  console.log("- ", user.name + " (" + countIncompleteTask + " incomplete task" + ")");
})
/**
