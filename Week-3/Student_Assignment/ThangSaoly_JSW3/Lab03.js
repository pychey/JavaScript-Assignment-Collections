...
const userData = [
    {
        ...
        tasks: [
            ...
            // { taskId: 103, description: 'Implement 2nd new feature', completed: false }
        ],
        contact: {
...
const fTask = userData[0];
console.log('First User: ',fTask.name);
console.log('First Task: ',fTask.tasks[0].description);
console.log('-------------------------------------------------------');
...
const userWithoutBob = userData.filter(n => n.name !== 'Bob')
console.log('Users other than Bob: ',userWithoutBob);
console.log('-------------------------------------------------------');
...
userData.forEach(user => {
    const incomplete = user.tasks.find(task => !task.completed);
    if(incomplete){
        const count = user.tasks.filter(task => !task.completed).length;
        console.log(`- ${user.name} (${count} incomplete task${count > 1 ? 's' : ''})`);    }
});
console.log('-------------------------------------------------------');
/**
...
const newTask = { taskId: 203, description: 'Mentor junior staff', completed: false };
const updatedUserData = userData.map(user => {
    if (user.id === 2) {
        return { ...user, role: 'Senior Manager', tasks: [...user.tasks, newTask]};
    }
    return user;
} );
