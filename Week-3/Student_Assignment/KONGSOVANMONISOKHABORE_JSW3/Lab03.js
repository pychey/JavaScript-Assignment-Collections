...
const [{name : firstUserName, tasks:[{description : firstTaskDescription}]}] = userData;
console.log("First User:", firstUserName);
console.log("First Task:", firstTaskDescription);
/**
...
const usersOtherthanBob = userData.filter(user => user.name !=='Bob');
console.log("Users other than Bob:", usersOtherthanBob);
// const usersOtherthanBob = userData.filter(({name}) => name !== 'Bob');
/**
...
const findIncompletedTask = userData.filter(user => user.tasks.find(task => task.completed == false));
console.log("User with incomplete tasks:", findIncompletedTask);
findIncompletedTask.forEach(user => {
    const countIncompletedTask = user.tasks.filter(task => !task.completed).length;
    console.log("- " + user.name +"(" + countIncompletedTask + " incomplete tasks )");
});
// const usersWithIncompleteTasks = userData.filter(({tasks}) => tasks.find(({completed}) => !completed));
    ...
    if(user.id === 2){
        return {
            ...
            tasks: [
                ...
                {
                    taskId: 203,
                    description: 'Mentor junior staff',
                    completed: false
                }
            ]
        }
