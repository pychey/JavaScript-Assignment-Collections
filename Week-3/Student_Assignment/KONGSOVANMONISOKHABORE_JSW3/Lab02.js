...
// Mock da
// ta representing products available in the store
const products = [
...
const found = products.find(products => products.name === 'Laptop' || products.id === 'p1');
...
const electronic = products.filter(p => p.category === 'Electronics');
if (!p){
...
let totalPrice = 0;
cart.forEach(cart => {const product = products.find(product => product.id === cart.productId);
    if (product){
        totalPrice+= product.price * cart.quantity;
    }

});
console.log('total cart price:price', totalPrice);
...
const updatedProducts = products.map (products => products.id === 'p1' ? {...products, price: 900} : products);
