...
// YOUR TASK 2 CODE HERE
  const newData = userData.filter(item => item.name !== "Bob");
  console.log(newData);
/**
...
const notComplete = userData.filter((item)=>item.tasks.find((task)=>task.completed==false));
notComplete.forEach(function(item){
    const quantityTask = item.tasks.length;
    const notCompleteTask = item.tasks.filter((i)=> i.completed==false);
    const grammar = quantityTask > 1 ? "tasks" : "task"
    console.log(`${item.name} has ${quantityTask} ${grammar},(incomplete ${notCompleteTask.length})`);
})
/**
...
const updatedUserData = userData.map((item)=>{
    if(item.id == 2){
       const newUser = {...item,role:"Senior Manager",tasks:[...item.tasks,{ taskId: 203, description: "Mentor junior staff", completed: false }]}
       return newUser;
    }
    return item;
})
