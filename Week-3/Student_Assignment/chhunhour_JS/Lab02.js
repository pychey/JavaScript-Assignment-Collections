...
const item = products.find((n) => n.name === "Laptop");
console.log(item);
...
const itemType = products.filter((n) => n.category === "Electronics");
console.log("Electronics products: ");
for (let i = 0; i < itemType.length; i++) {
  console.log("_", itemType[i].name);
}
...
let price = 0;
cart.forEach((n) => {
  const found = products.find((i) => n.productId === i.id);
  if (found) {
    price += found.price * n.quantity;
  }
});
