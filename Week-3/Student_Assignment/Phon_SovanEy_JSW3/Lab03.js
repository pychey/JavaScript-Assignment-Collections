...
const noneBob = userData.filter((user) => user.name !== "Bob");
console.log("Users other than Bob:", JSON.stringify(noneBob, null, 2));
/**
...
const incompleteTasks = userData.filter((incomplete) =>
  incomplete.tasks.find((task) => task.completed == false)
);
console.log("Users with incomplete tasks: ");
incompleteTasks.forEach((user) => {
  const incompleteTasksCount = user.tasks.filter(
    (count) => count.completed == false).length;
  console.log(user.name + " (" + incompleteTasksCount + " incomplete task)");
});
