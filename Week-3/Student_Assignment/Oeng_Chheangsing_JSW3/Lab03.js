...
const { name: firstUserName, tasks: [firstTask] } = firstUser;
...
const userBob = userData.filter(n => n.name !== 'Bob')
console.log('Users other than Bob:', userBob);
...
const userNotCompleted = userData.filter(m => m.tasks.find(t => !t.completed))
console.log('Users with incomplete tasks:')
userNotCompleted.forEach(i => console.log('-', i.name))
...
console.log('Updated User Data:', JSON.stringify(check, null, 2));